...
plt.xlim([-500, 10500])
plt.ylim([-500, 10500])
plt.scatter(x_leak, y_leak, s=40, marker='X', color='#BEB8DC', label='Sources')

type_marker = ['o', 'v', '^', '<', '>', '8', 's', 'p', '*', 'h', 'H', 'D', 'd', 'P', 'X']
# plot grid strategy

geox, geoy, geoz = psarray.gridarray(9, 10000, 10000)  # extract sensor in placed positions
plt.scatter(geox, geoy, s=100, marker='P', color='#FFBE7A', label='Regular network')

plt.scatter(x_so_place, y_so_place, s=80, marker='o', color='#FA7F6F', label='Detection time Optimization')

plt.scatter(geox_so_loc, geoy_so_loc, s=60, marker='*', color='#8ECFC9', label='Source localization Optimization')
