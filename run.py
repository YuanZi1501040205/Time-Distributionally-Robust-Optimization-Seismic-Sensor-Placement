...
def leak_simulation(grid, atm, leak_positions, leak_heights, leak_rates, event_name='S'):
    ...
    for i, (leak_point, leak_h, leak_r) in enumerate(zip(leak_positions, leak_heights, leak_rates)):
        print('simulation: ', i)
        ...
        signals[scenario_name] = signal[event_name]
        print(signal.head(5))
...
def eval_sensor_placement(min_det_time, sensor, scenario, placed_sensor_set):
    #  optimization impact formulation
    impactform = chama.optimize.ImpactFormulation()
...
# %% optimize based on the mean wind time and test on the test data
...
print('sensor placement for mean wind: ', opt_result_m['Objective'])
covered_scenario_number_m = int(np.where(opt_result_m['Assessment']['Sensor'].values == None)[0][0])
total_scenario_number = opt_result_m.__len__()
print('covered_scenario_number_m: ', covered_scenario_number_m)
print('total_scenario_number: ', total_scenario_number)
# %% optimize based on the DRO modified detect time and test on the test data

for i in tqdm(range(num_test_sample)):
    ...
    signals, scenario = leak_simulation(grid, atm, leak_positions, leak_heights, leak_rates, event_name='S'+str(i)+'_')
    min_det_time = extract_min_detect_time(signals, sensors)
    if i == 0:
        min_det_time_samples = min_det_time
    else:
        min_det_time_samples.append(min_det_time)
